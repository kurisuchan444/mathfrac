...
export default class SketchPad extends Component {
  ...
  onTouchStart(e) {
    const data = this.tool.onMouseDown(...this.getTouchPosition(e), this.props.color, this.props.size, this.props.fillColor);
    data && data[0] && this.props.onItemStart && this.props.onItemStart.apply(null, data);
    if (this.props.onDebouncedItemChange) {
      this.interval = setInterval(this.onDebouncedMove, this.props.debounceTime);
    }
  ...
  onTouchMove(e) {
    const data = this.tool.onMouseMove(...this.getTouchPosition(e));
    data && data[0] && this.props.onEveryItemChange && this.props.onEveryItemChange.apply(null, data);
    e.preventDefault();
  }
  ...
  onTouchEnd(e) {
    const data = this.tool.onMouseUp(...this.getTouchPosition(e));
    data && data[0] && this.props.onCompleteItem && this.props.onCompleteItem.apply(null, data);
    if (this.props.onDebouncedItemChange) {
      clearInterval(this.interval);
      this.interval = null;
  ...
  render() {
    ...
    return (
      <canvas
        ...
        width={width}
        height={height}
        style={{backgroundColor:this.props.background,touchAction:'none',width:width,height:height}}
      />
    )
  }
}
